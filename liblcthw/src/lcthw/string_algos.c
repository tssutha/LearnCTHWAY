		...
		size_t *skip_chars)
{
	...
	assert(haystack != NULL && "Given bad heystack to search");
...
StringScanner *StringScanner_create(bstring in)
{
	...
	printf("scan : %p\n", scan);
	printf("Scan Create : Len =%d\n", (int)scan->hlen);
	assert(scan != NULL && "scan cannot be NULL");
	return scan;
error:
	free(scan);
	return NULL;
...
{	
	const unsigned char *found = NULL;
	ssize_t found_at = 0;
	//printf("Step 0  : hlen = %d\n", (int)scan->hlen);
	//assert( scan != NULL && "fuckkkkkk shit");
