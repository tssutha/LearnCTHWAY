...
Hashmap *Hashmap_create(Hashmap_compare cmp, Hashmap_hash hash)
{
	...
	map->compare = cmp != NULL? cmp: default_compare;
	map->hash    = hash != NULL? hash: default_hash;
	map->buckets = DArray_create(sizeof(DArray*), DEFAULT_NUM_OF_BUCKETS);
	map->buckets->end = map->buckets->max; //fake it expanding it
										  ...
										  int create, uint32_t *hash_out)
{
	...
	int bucket_n = hash % DEFAULT_NUM_OF_BUCKETS;
	...
	*hash_out = hash; //store it for the return so caller can use it
	...
	if(!bucket && create){
		...
		bucket = DArray_create(sizeof(void *), DEFAULT_NUM_OF_BUCKETS);
		check_mem(bucket);
