...
enum { ALGO_FNV1A, ALGO_ADLER32, ALGO_DJB, ALGO_JKIN };
...
// 	
	...
	int ret = gen_keys(keys, NUM_KEYS);
	mu_assert(ret == 0, "Failed to generate random keys");
	...
	fill_distribution(stats[ALGO_JKIN], keys, Hashmap_jenkins_hash);
	fprintf(stderr, "FNV\tA32\tDJB\tJKIN\n");
