...
char *test_delete()
{
	...
	mu_assert(deleted == &expect1, "should get THE VALUE 1");
	bstring result = BSTree_get(map, &test1);
	mu_assert(result == NULL , "should be deleted");
	
	deleted = (bstring)BSTree_delete(map, &test2);
	mu_assert(deleted != NULL , "got NULL on delete");
	mu_assert(deleted == &expect2, "should get THE VALUE 2");
	result = BSTree_get(map, &test2);
	mu_assert(result == NULL, "should be deleted");
	
	deleted = (bstring)BSTree_delete(map, &test3);
	mu_assert( deleted != NULL , " got NULL on delete");
	mu_assert( deleted == &expect3, "should get THE VALUE 3");
	result = BSTree_get(map, &test3);
	mu_assert(result == NULL, "should be deleted");
	
	//test deleting non-existent stufff
	deleted = (bstring)BSTree_delete(map, &test3);
...
char *test_fuzzing()
{
	...
	for(i=0; i < 100; i++){
		...
		mu_assert(value == data[i], "Failed to delete the right node");
		mu_assert(BSTree_delete(store, numbers[i]) == NULL, "should get NULL");
		
		// for(j= i+1;j < 99-i; j++){
// 			bstring value = BSTree_get(store, numbers[j]);
// 			//mu_assert(value == data[j], "Failed to get the right data");
// 		}
